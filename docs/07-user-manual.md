...
- **Team** — invite teammates by email, change a member's role, or remove them (an org needs at least one Admin, so the last one can't be demoted or removed). Inviting sends them a real email with a link to verify and log in — click it, and they land straight in your organization, no separate signup step needed. The one case that doesn't send an email: if that address already has a GuardrunAgent/Supabase account (most often someone you removed earlier and are now re-inviting), the list marks that row **"(no email sent)"** — just tell them directly (Slack, however you'd normally reach them) to sign in with that same email address (Google or email/password), and they'll be linked to your organization automatically the moment they do.
