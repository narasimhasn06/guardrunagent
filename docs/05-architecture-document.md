...
| Backend unreachable during a guardrail check | Fail-open vs. fail-closed is a real **org-level setting** (`orgs.fail_mode`, docs/03-low-level-design.md Section 1), configurable from the Settings page (docs/04-ui-ux-design.md Section 3.6) and defaulting to fail-open (never block the user's own work over a networking blip). The SDK fetches it alongside `GET /rules` and caches it the same way (see `sdk/src/ruleCache.ts`) — there's no persistent SDK process to hold it otherwise (each hook invocation is a fresh process). `GUARDRUNAGENT_FAIL_MODE` / `~/.guardrunagent/config.json` (`sdk/src/config.ts`) still exist, now as a per-machine override in front of the org setting rather than the only source of truth. |
