...
- **Team/Users:** simple list + invite-by-email, role toggle (Admin/Member). "No granular permissions needed at MVP" means exactly two levels, not that a Member has the same team-management powers as an Admin: inviting, cancelling an invite, and changing anyone's role (including self-promotion) are **Admin-only**, enforced on the backend (`app/routers/settings.py`'s `_require_admin`) and reflected in the UI (a Member sees the same list, read-only — no invite form, role toggle, or cancel-invite button). This was a real gap fixed during implementation, not the original design — see CLAUDE.md's decisions log. An org must always keep at least one Admin: demoting the last one is rejected (409) — this specifically catches an Admin demoting *themselves*, which would otherwise remove their own access to these very controls with no way back. The dashboard disables the toggle on that row pre-emptively; the backend check is still the real gate. Invited members sign in via the same Login screen (email/password or Google), and are linked to the org on first login.
